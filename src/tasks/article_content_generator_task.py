...
class ArticleContentGeneratorTask(BaseTaskModule):
    ...
    def _generate_article_content(self, article_concept: Dict[str, Any], source_content: str, blog: Dict[str, Any]) -> tuple[str, str]:
        ...
        prompt = (
            ...
            "【重要：入力データのクリーニング指示】\n"
            "提供された【参考情報】には、チャットボットのタスクログ、システムメッセージ、デバッグ情報、または冗長な繰り返しテキストが含まれている場合があります。\n"
            "これら「記事の構成要素ではないノイズ」は徹底的に無視・除外し、記事のテーマに合致する有益な情報のみを抽出して執筆に使用してください。\n"
            "特に「タスクログ」「実行履歴」のようなメタデータは、絶対にブログ記事の本文に含めないでください。\n\n"
            f"【ブログ情報】\n名前: {blog_name}\n説明: {blog_description}\n"
